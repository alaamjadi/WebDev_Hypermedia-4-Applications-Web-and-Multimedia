  ...
  fetch(URL.API + URL.EVENTS + URL.YEAR + year + URL.MONTH + month)
    ...
    .then(function (data) {
      data.forEach((element) => {
        ...
        document.getElementById("event_cards").innerHTML += `<div class="mb-4 align-items-start col-12 col-sm-6 col-md-6 col-lg-4 col-xl-4"><div class="cus_card"><a href="event.html?event_id=${element.event_id}"><div class="card shadow mb-5 bg-white rounded-lg"><div class="view overlay"><img class="card-img-top" src="../assets/img/${pic[0]}" alt="Event card photo ${element.event_id}"/></div><div class="card-body" id="card_events"><h3 class="card-title mb-0 mt-0 text-center"> ${element.event_name} </h3><p class = "text-center mt-2 mb-0 short_descr"> ${element.event_date.slice(0,10)} </p><p class = "text-center mt-0 short_descr"> ${element.event_date.slice(11,16)} </p><p class = "text-center short_descr"> ${element.event_location}</p><p class = "text-center mt-2 short_descr"> ${element.short_description}</p><!-- <p class="card-text"></p> --></div></div></a></div></div>`;});
      eventsInit()
    })
