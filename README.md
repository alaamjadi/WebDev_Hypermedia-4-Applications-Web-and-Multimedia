...
**Note regarding Render services:** The Services, Events, and Staff web pages pull content via API calls from the PostgreSQL database. Since the current render plan includes a one-month free trial, content on these pages may not load after the trial period ends.
...
![Backend Structure](public/backend/img/backend-structur.jpg "Backend Structure")
